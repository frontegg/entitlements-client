...
describe(SpiceDBQueryClient.name, () => {
	...
	it.each([
		...
		await queryClient.spiceDBQuery(subjectContext, requestContext, LEGACY_NAMESPACE);

		expect(mockStrategy).toHaveBeenCalledWith({ requestContext, subjectContext }, LEGACY_NAMESPACE);
	});
});
