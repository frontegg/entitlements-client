...
describe(EntitlementsClient.name, () => {
	describe.each(Object.values(RequestContextType))('Given monitoring mode in opa is disabled for `%s` request context type', (requestContextType) => {
		// GIVEN
	...
	describe.each(Object.values(RequestContextType))('Given monitoring mode in opa is turned on for `%s` request context type', (requestContextType) => {
		// GIVEN
		const mockOpaQueryClient: MockProxy<EntitlementsOpaQuery> = mock<EntitlementsOpaQuery>();
		const mockLoggingClient: MockProxy<LoggingClient> = mock<LoggingClient>();
		const opaResult: OpaResponse<EntitlementsResult> = {
