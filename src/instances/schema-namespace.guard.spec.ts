...
describe('schema namespace guard', () => {
	...
	it('should build every SpiceDB request object type through namespace.type()', () => {
		...
		for (const file of sources) {
			...
			while ((match = OBJECT_TYPE_FIELD.exec(source)) !== null) {
				...
				if (value.startsWith('namespace.type(') || value === 'string') {
					continue;
				}
